...
namespace Generator.Sources.ObservableEvents.EventGenerators;
...
internal static class RoslynGeneratorExtensions
{
    ...
    public static IReadOnlyCollection<ArgumentSyntax> GenerateArgumentList(
        this IParameterSymbol parameter
    ) => [Argument(parameter.Name.GetKeywordSafeName())];
    ...
    /// <returns>The argument list.</returns>
    public static IReadOnlyCollection<ArgumentSyntax> GenerateTupleArgumentList(
        this IEnumerable<IParameterSymbol> parameters
    ) =>
        [
            Argument(
                TupleExpression(
                    parameters.Select(x => Argument(x.Name.GetKeywordSafeName())).ToList()
                )
            ),
        ];

    public static TypeSyntax GenerateTupleType(
        this IReadOnlyCollection<(ITypeSymbol Type, string Name)> typeDescriptors
    )
    {
            ...
            argumentList = TypeArgumentList(
                [
                    TupleType(
                        method
                            .Parameters.Select(x =>
                                TupleElement(x.Type.GenerateFullGenericName(), x.Name)
                            )
                            .ToList()
                    ),
                ]
    ...
    public static IEnumerable<IEventSymbol> GetEvents(
        this INamedTypeSymbol item,
        Func<string, ITypeSymbol?> getSymbolOf,
        bool staticEvents = false,
        bool includeInherited = true
    )
    {
        var baseClassWithEvents = includeInherited
            ? item.GetBasesWithCondition(RoslynHelpers.HasEvents)
            : Array.Empty<INamedTypeSymbol>();

        var itemsToProcess = new Stack<INamedTypeSymbol>(
            new[] { item }.Concat(baseClassWithEvents)
        );
        ...
        while (itemsToProcess.Count != 0)
        {
            ...
            for (int memberIndex = 0; memberIndex < members.Length; memberIndex++)
            {
                ...
                var invokeMethod = (
                    (INamedTypeSymbol)eventSymbol.OriginalDefinition.Type
                ).DelegateInvokeMethod;

                if (
                    invokeMethod == null
                    || ((invokeMethod.ReturnType as INamedTypeSymbol)?.IsGenericType ?? false)
                )
                {
                    continue;
                }
                ...
                if (
                    invokeMethod.ReturnType.SpecialType == SpecialType.System_Void
                    || SymbolEqualityComparer.Default.Equals(
                        invokeMethod.ReturnType,
                        getSymbolOf(typeof(Task).FullName)
                    )
                    || SymbolEqualityComparer.Default.Equals(
                        invokeMethod.ReturnType,
                        getSymbolOf(typeof(ValueTask).FullName)
                    )
                )
                {
                    yield return eventSymbol;
    ...
    public static TypeSyntax GenerateObservableType(this TypeArgumentListSyntax argumentList)
    {
        return QualifiedName(
            ...
            GenericName("IObservable").WithTypeArgumentList(argumentList)
        );
    }
    ...
    public static TypeSyntax GenerateObservableType(this TypeSyntax argumentList)
    {
        return QualifiedName(
            IdentifierName("global::System"),
            GenericName("IObservable", [argumentList])
        );
    }
    ...
    public static IReadOnlyList<ParameterSyntax> GenerateMethodParameters(this IMethodSymbol method)
    {
        ...
        return method
            .Parameters.Select(x =>
                Parameter(x.Type.GenerateFullGenericName(), x.Name.GetKeywordSafeName())
            )
            .ToList();
    }
    ...
    public static IReadOnlyList<TypeParameterSyntax> ToTypeParameters(
        this IEnumerable<INamedTypeSymbol> types
    )
    {
    ...
    public static IEnumerable<INamedTypeSymbol> GetBasesWithCondition(
