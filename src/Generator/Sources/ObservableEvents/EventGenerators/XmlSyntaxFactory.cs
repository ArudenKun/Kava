...
internal static class XmlSyntaxFactory
{
    ...
    public static SyntaxTriviaList GenerateSummaryComment(
        string summaryText,
        string parameterFormat,
        IMethodSymbol entity
    )
    {
        var parameters = entity.Parameters.Select(x =>
            (x.Name, string.Format(CultureInfo.InvariantCulture, parameterFormat, x.Name))
        );
    ...
    /// <returns>The syntax trivia of the comment.</returns>
    public static SyntaxTriviaList GenerateSummarySeeAlsoComment(
        string summaryText,
        string seeAlsoText
    )
    {
        var text = string.Format(
            ...
            summaryText,
            "<see cref=\"" + seeAlsoText.Replace("<", "{").Replace(">", "}") + "\" />"
        );
        var template =
            "/// <summary>" + _newLine + $"/// {text}" + _newLine + "/// </summary>" + _newLine;
    ...
    public static SyntaxTriviaList GenerateSummarySeeAlsoComment(
        string summaryText,
        string seeAlsoText,
        params (string ParamName, string ParamText)[] parameters
    )
    {
        var text = string.Format(
            CultureInfo.InvariantCulture,
        ...
        foreach (var parameter in parameters)
        {
            sb.Append("/// <param name=\"")
                .Append(parameter.ParamName)
                .Append("\">")
                .Append(parameter.ParamText)
                .AppendLine("</param>");
        }
    ...
    {
        var stringBuilder = new StringBuilder(
            method.ContainingType.GetArityDisplayName() + "." + method.Name
        ).Append('(');
