...
internal static class RoslynHelpers
{
    ...
    public static SymbolDisplayFormat SymbolDisplayFormat { get; } =
    ...
    public static SymbolDisplayFormat GenericTypeFormat { get; } =
        new SymbolDisplayFormat(
    ...
    public static SymbolDisplayFormat TypeFormat { get; } =
        new SymbolDisplayFormat(
            globalNamespaceStyle: SymbolDisplayGlobalNamespaceStyle.Included,
            typeQualificationStyle: SymbolDisplayTypeQualificationStyle.NameAndContainingTypesAndNamespaces,
    ...
    /// </summary>
    public static ArgumentSyntax[] ReactiveUnitArgumentList { get; } =
        [Argument(ObservableUnitName + ".Default")];

    public static Func<INamedTypeSymbol, bool> HasEvents { get; } =
        symbol =>
            symbol
                .GetMembers()
                .OfType<IEventSymbol>()
                .Any(x => x.DeclaredAccessibility == Accessibility.Public);
    ...
    {
        var obsoleteAttribute = eventDetails
            .GetAttributes()
            .FirstOrDefault(attr =>
                attr.AttributeClass?.GetArityDisplayName()
                    .Equals("System.ObsoleteAttribute", StringComparison.InvariantCulture) ?? false
            );
        ...
        var isError = bool.Parse(
            obsoleteAttribute.ConstructorArguments.ElementAtOrDefault(1).Value?.ToString()
                ?? bool.FalseString
        )
        ...
        if (message != null && !string.IsNullOrWhiteSpace(message))
        {
            var attribute = Attribute(
                ...
                [
                    ...
                    AttributeArgument(LiteralExpression(isError)),
                ]
            );
    ...
    )
    {
        ...
        if (typeSymbol is INamedTypeSymbol namedTypeSymbol && namedTypeSymbol.TypeParameters.Any())
        {
            return GenericName(
                typeName,
                genericTypeParameters ?? namedTypeSymbol.GetTypeParametersAsTypeSyntax()
            );
        }
            ...
            .TypeParameters.Select(tp =>
                (
                    ...
                    Constraints: tp.GetOtherConstrainst()
                        ...
                        .Select(tpc =>
                            tpc.WithTrailingTrivia(SyntaxFactory.Space).WithoutLeadingTrivia()
                        )
                        .ToArray()
                )
            )
    ...
    {
        if (typeSymbol.HasReferenceTypeConstraint || typeSymbol.HasValueTypeConstraint)
        {
    ...
    private static TypeParameterConstraintSyntax GetAsTypeConstraint(this ITypeSymbol typeSymbol)
    {
        if (typeSymbol is INamedTypeSymbol namedTypeSymbol && namedTypeSymbol.TypeArguments.Any())
        {
            return TypeConstraint(
                namedTypeSymbol.GetTypeSyntax(
                    genericTypeParameters: namedTypeSymbol
                        .TypeArguments.Select(tp => tp.GenerateFullGenericName())
    ...
    {
        return typeSymbol.TypeParameters.Select(tp => tp.Name).Select(parseName).ToArray();
    }
}
