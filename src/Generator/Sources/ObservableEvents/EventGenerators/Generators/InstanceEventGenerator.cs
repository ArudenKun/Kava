...
using System.Linq;
using Generator.Sources.ObservableEvents.EventGenerators.Comparers;
...
internal class InstanceEventGenerator : EventGeneratorBase
{
    ...
    {
        var namespaceName = item.ContainingNamespace.ToDisplayString(
            RoslynHelpers.SymbolDisplayFormat
        );

        var eventWrapperes = GenerateEventWrapperClasses(
                item,
                item.GetEvents(getSymbolOf).ToArray()
            )
            .ToList();
    ...
    private static ConstructorDeclarationSyntax GenerateEventWrapperClassConstructor(
    ...
    )
    {
        ...
        return ConstructorDeclaration(
                ...
                [Parameter(typeDefinition.GetTypeSyntax(), dataParameterName)],
                className,
                constructorBlock,
                1
            )
            .WithLeadingTrivia(
                XmlSyntaxFactory.GenerateSummarySeeAlsoComment(
                    ...
                    className,
                    (dataParameterName, "The class that is being wrapped.")
                )
            );
    }
    ...
    private static IEnumerable<ClassDeclarationSyntax> GenerateEventWrapperClasses(
        INamedTypeSymbol typeDefinition,
    ...
    )
    {
        ...
        {
            GenerateEventWrapperField(typeDefinition),
            GenerateEventWrapperClassConstructor(typeDefinition),
        };
