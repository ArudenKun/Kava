...
{
    private static readonly QualifiedNameSyntax _subjectNamespace = QualifiedName(
        "Pharmacist",
        "Common"
    );
    ...
    internal static IEnumerable<NamespaceDeclarationSyntax> Generate(
        IEnumerable<(
    ...
    {
        foreach (
            var groupedDeclarations in declarations
                .GroupBy(x => x.TypeDefinition.ContainingNamespace.Name)
                .OrderBy(x => x.Key)
        )
        {
            ...
            members.AddRange(
                groupedDeclarations
                    ...
                    .Select(x => GenerateClass(x.TypeDefinition, x.IsAbstract, x.Methods))
            );
    ...
    {
        var modifiers =
            typeDefinition.IsAbstract || isAbstract
        ...
        return ClassDeclaration(
                typeDefinition.Name + "Rx",
                attributes,
                modifiers,
                GenerateObservableMembers(methods).ToList(),
                baseClasses,
                1
            )
            .WithLeadingTrivia(
                XmlSyntaxFactory.GenerateSummarySeeAlsoComment(
                    "Wraps delegates events from {0} into Observables.",
                    typeDefinition.GetArityDisplayName()
                )
            );
    }
    ...
    )
    {
        ...
        {
            var observableName =
                "_" + char.ToLowerInvariant(method.Name[0]) + method.Name.Substring(1);
    ...
    )
    {
        ...
        return PropertyDeclaration(
                method.GenerateObservableTypeArguments().GenerateObservableType(),
                method.Name + "Obs",
                attributes,
                modifiers,
                ArrowExpressionClause(IdentifierName(observableName)),
                2
            )
            .WithLeadingTrivia(
                XmlSyntaxFactory.GenerateSummarySeeAlsoComment(
                    "Gets an observable which signals when the {0} method is invoked.",
    ...
    )
    {
        ...
        var typeName = QualifiedName(
            _subjectNamespace,
            _subjectType.WithTypeArgumentList(method.GenerateObservableTypeArguments())
        );
    ...
    )
    {
        ...
        var methodBody = ArrowExpressionClause(
