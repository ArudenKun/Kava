...
using static Generator.Sources.ObservableEvents.SyntaxFactoryHelpers;

namespace Generator.Sources.ObservableEvents.EventGenerators.Generators;
...
{
    /// <inheritdoc />
    public abstract NamespaceDeclarationSyntax? Generate(
    ...
    protected static PropertyDeclarationSyntax? GenerateEventWrapperObservable(
        IEventSymbol eventDetails,
    ...
    )
    {
        ...
        var (expressionBody, observableEventArgType) = GenerateFromEventExpression(
            eventDetails,
            dataObjectName
        );
        ...
        return PropertyDeclaration(
                observableEventArgType,
                prefix + eventDetails.Name,
                attributes,
                modifiers,
                expressionBody,
                2
            ...
            .WithLeadingTrivia(
                XmlSyntaxFactory.GenerateSummarySeeAlsoComment(
                    "Gets an observable which signals when the {0} event triggers.",
                    eventDetails.ConvertToDocument()
                )
            )
    ...
    ) GenerateFromEventExpression(IEventSymbol eventSymbol, string dataObjectName)
    {
        ...
        if (
            ...
            && invokeMethod.Parameters[0].Type.GenerateFullGenericName() == "object"
        )
        {
            methodParametersArgumentList = invokeMethod.Parameters[1].GenerateArgumentList();
            eventArgsType = IdentifierName(
                invokeMethod.Parameters[1].Type.GenerateFullGenericName()
            );
        }
        else if (invokeMethod.Parameters.Length > 0)
        {
            ...
            methodParametersArgumentList =
                invokeMethod.Parameters.Length == 1
                    ? invokeMethod.Parameters[0].GenerateArgumentList()
                    : invokeMethod.Parameters.GenerateTupleArgumentList();
            eventArgsType =
                invokeMethod.Parameters.Length == 1
                    ? IdentifierName(invokeMethod.Parameters[0].Type.GenerateFullGenericName())
                    : invokeMethod
                        .Parameters.Select(x => (x.Type, x.Name))
                        .ToList()
                        .GenerateTupleType();
        }
        else
        ...
        var localFunctionExpression = GenerateLocalHandler(
            methodParametersArgumentList,
            invokeMethod,
            "obs.OnNext",
        ...
        var expression = ArrowExpressionClause(
            InvocationExpression(
                MemberAccessExpression(
                    SyntaxKind.SimpleMemberAccessExpression,
                    "global::R3.Observable",
                    GenericName("Create", [eventArgsType])
                ),
    ...
    private static LocalFunctionStatementSyntax GenerateLocalHandler(
        IReadOnlyCollection<ArgumentSyntax> methodParametersArgumentList,
        ...
        LocalFunctionStatement(
            ...
            returnType.SpecialType == SpecialType.System_Void
                ? GenerateArrowExpressionClauseForSyncronousHandler(
                    handlerName,
                    methodParametersArgumentList
                )
                : default,
            returnType.SpecialType != SpecialType.System_Void
                ? GetBlockSyntaxForAsynconousHandler(
                    handlerName,
                    methodParametersArgumentList,
                    returnType
                )
    ...
    private static ArrowExpressionClauseSyntax GenerateArrowExpressionClauseForSyncronousHandler(
        string handlerName,
        IReadOnlyCollection<ArgumentSyntax> methodParametersArgumentList
    ) => ArrowExpressionClause(InvocationExpression(handlerName, methodParametersArgumentList));

    private static BlockSyntax GetBlockSyntaxForAsynconousHandler(
        string handlerName,
        IReadOnlyCollection<ArgumentSyntax> methodParametersArgumentList,
        ITypeSymbol returnType
    ) =>
    ...
    private static ExpressionSyntax GetExpressionSyntax(ITypeSymbol returnType)
    {
        if (returnType.Name == nameof(Task))
        {
            return MemberAccessExpression(
                SyntaxKind.SimpleMemberAccessExpression,
                returnType.GenerateFullGenericName(),
                IdentifierName(nameof(Task.CompletedTask))
            );
        }
        else if (returnType.Name == nameof(ValueTask))
        {
            return SyntaxFactory.DefaultExpression(
                IdentifierName(returnType.GenerateFullGenericName())
            );
        }

        throw new NotSupportedException(
            $"The return type of {returnType.GenerateFullGenericName()} is not supported!"
        );
    }

    private static AssignmentExpressionSyntax GenerateEventAssignment(
        SyntaxKind accessor,
        string eventName,
        string dataObjectName,
        string handlerName
