...
public sealed class WebView2Adapter : IWebViewAdapter
{
    ...
    public WebView2Adapter(string? userDataDir = null) => _userDataDir = userDataDir;
    ...
    public async Task SetParentAsync(IntPtr handle)
    {
        ...
        if (_controller == null)
        {
            var env = await CoreWebView2Environment.CreateAsync(userDataFolder: _userDataDir);
    ...
    private Action AddHandlers(CoreWebView2 webView)
    {
        ...
        void WebViewOnNavigationCompleted(
            object? sender,
