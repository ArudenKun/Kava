...
public static class Lambda
{
    ...
    public static TStruct? TryOrNull<TStruct>(Func<TStruct?> function)
    ...
    public static TClass? TryOrNull<TClass>(Func<TClass> function)
        where TClass : class
    {
        try
        {
    ...
    public static async Task<TClass?> TryOrNullAsync<TClass>(Func<Task<TClass>> function)
        where TClass : class
    {
        try
        {
    ...
    /// <summary>
    ///     Attempts to run <paramref name="function"/> in a <see langword="try"/> block.
    ///     If an error of type <typeparamref name="TException"/> occurs, <paramref name="onError"/> is called.
    /// </summary>
    /// <param name="function">The function to run.</param>
    /// <param name="onError">The function called on failure.</param>
    /// <typeparam name="TException">The type of exception to catch.</typeparam>
    public static void TryCatch<TException>(Action function, Action<TException> onError)
    ...
    /// <typeparam name="T">The type of the object being returned from either <paramref name="function"/> or <paramref name="onError"/>.</typeparam>
    /// <returns>The value from <paramref name="function"/>; or <paramref name="onError"/>'s value if <paramref name="function"/> throws an <see cref="Exception"/>.</returns>
    public static Task<T> TryCatchAsync<T>(
    ...
    /// <typeparam name="TException">The specific type of exception to catch. All other exceptions except for one of this type *will* cause a throw.</typeparam>
    /// <returns>The value from <paramref name="function"/>; or <paramref name="onError"/>'s value if <paramref name="function"/> throws an <see cref="Exception"/>.</returns>
    public static async Task<T> TryCatchAsync<T, TException>(
        Func<Task<T>> function,
    ...
    /// <param name="onError">The function called on failure.</param>
    /// <typeparam name="TException">The type of exception to catch.</typeparam>
    public static async Task TryCatchAsync<TException>(
        Func<Task> function,
    ...
    public static void Repeat([NonNegativeValue] int times, Action action)
    {
    ...
    public static async Task RepeatAsync([NonNegativeValue] int times, Func<Task> function)
    {
        for (var i = 0; i < times; i++)
            await function();
    }
