...
// ReSharper restore CheckNamespace
{
    ...
    )
    {
        ...
        builder.Configure(c =>
            c.Journal = new SQLiteTableJournal(
                () => c.ConnectionManager,
                () => c.Log,
                "SchemaVersions"
            )
        ...
        builder.Configure(c =>
            c.ScriptExecutor = new SQLiteScriptExecutor(
                () => c.ConnectionManager,
                () => c.Log,
                null,
                () => c.VariablesEnabled,
    ...
    public static UpgradeEngineBuilder JournalToSQLiteTable(
        this UpgradeEngineBuilder builder,
        string table
    )
    {
        builder.Configure(c =>
            c.Journal = new SQLiteTableJournal(() => c.ConnectionManager, () => c.Log, table)
        );
        return builder;
    }
}
