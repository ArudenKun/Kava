...
public sealed class NativeWebView : NativeControlHost, IWebView, IDisposable
{
    private static readonly Uri EmptyPageLink = new("about:blank");
    private static Func<IWebViewAdapter>? _webViewAdapterFactory;

    private readonly IWebViewAdapter _webViewAdapter;

    public static void SetFactory(Func<IWebViewAdapter> webViewAdapterFactory) =>
    ...
    {
        _webViewAdapter =
            _webViewAdapterFactory?.Invoke()
            ?? throw new NullReferenceException("IWebViewAdapter factory is not set");
